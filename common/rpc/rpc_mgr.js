...
RpcMgr.prototype.getAreaInfoList = async function () {
    let server = 'http-server0'
    if (SERVER_NAME == server) {
        //本服操作
        return await logic_mgr.getAreaInfoList();
    }
    else {
        //跨服操作
        return (await center_mgr.rpcAsync(server, "getAreaInfoList", {})).areaInfos
    }
}
